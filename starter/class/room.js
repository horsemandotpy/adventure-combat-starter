class Room {
  ...
  getItemByName(name) {
    // Fill this in
    // Find item in this.items array
    // If have the item name
      // Return the item object
    // Else return statement
      // That item not here, find elsewhere
    let itemTarget = this.items.find(item => item.name === name)

    if (itemTarget) {
      return itemTarget;
    } else {
      console.log(`This item ${name} is not here, find somewhere else.`)
    }
  }

  getEnemyByName(name) {
    // Fill this in
    // Get all enemy in currentRoom
